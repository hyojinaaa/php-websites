...
<form action="index.php?page=account" method="post">
		...
		<input type="text" name="first-name" value="<?= isset($_POST['update-contact']) ? $_POST['first-name'] : '' ?>"></input>
	
		<?= isset($firstNameMessage) ? $firstNameMessage : '' ?>

		<label for="">Last Name: </label>
		<input type="text" name="last-name" value="<?= isset($_POST['update-contact']) ? $_POST['last-name'] : '' ?>"></input>

		<?= isset($lastNameMessage) ? $lastNameMessage : '' ?>

		<input type="submit" class="button" name="update-contact" value="Update your name"></input>
...
<form action="index.php?page=account" method="post" enctype="multipart/form-data">
	...
	<div>
		...
		<textarea name="desc" id="desc" cols="30" rows="30"></textarea>
